...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculatively read ahead 16 bytes. This covers the token type + most fixed-size
    // payloads (e.g. Int32, Float64, Key, String length, short Number lengths) without
    // incurring a second asynchronous microtask.
    ...
    const getPayload = async (start: number, len: number) => {
      if (chunk.length >= start + len) {
        return chunk.subarray(start, start + len);
      }
      return await this.readBytes(absoluteOffset + BigInt(start), len);
    };
