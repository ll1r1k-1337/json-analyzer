...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculative read: fetch 16 bytes. Most tokens are small (e.g. 1-9 bytes).
    // This avoids multiple roundtrips and async microtask overhead.
    ...
    switch (type) {
      ...
      case TokenType.String: {
        let payload = chunk.subarray(1, 5);
        if (payload.length < 4) {
      ...
      case TokenType.Number: {
        let lengthBytes = chunk.subarray(1, 5);
        if (lengthBytes.length < 4) {
            lengthBytes = await this.readBytes(absoluteOffset + 1n, 4);
            if (lengthBytes.length < 4) throw new Error("Unable to read number length");
        }
        const byteLength = lengthBytes.readUInt32LE(0);

        let numberBytes = chunk.subarray(5, 5 + byteLength);
      ...
      case TokenType.Uint8: {
        let payload = chunk.subarray(1, 2);
        if (payload.length < 1) {
      ...
      case TokenType.Uint16: {
        let payload = chunk.subarray(1, 3);
        if (payload.length < 2) {
      ...
      case TokenType.Float64: {
        let payload = chunk.subarray(1, 9);
        if (payload.length < 8) {
      ...
      case TokenType.Float64Array: {
          ...
          let data = chunk.subarray(5, 5 + byteLength);
