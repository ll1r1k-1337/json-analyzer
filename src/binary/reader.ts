...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculative read optimization:
    // Read up to 16 bytes speculatively to avoid multiple await calls for common tokens
    const SPECULATIVE_READ_SIZE = 16;
    const speculativeBuffer = await this.readBytes(absoluteOffset, SPECULATIVE_READ_SIZE);
    ...
    // Helper to get payload, either from speculative buffer or by doing an additional read
    const getPayload = async (requiredBytes: number, skip: number = 1): Promise<Buffer> => {
      if (speculativeBuffer.length >= skip + requiredBytes) {
        return speculativeBuffer.subarray(skip, skip + requiredBytes);
      }
      return this.readBytes(absoluteOffset + BigInt(skip), requiredBytes);
    };
    ...
    switch (type) {
      ...
      case TokenType.String: {
        const payload = await getPayload(4);
        if (payload.length < 4) {
      ...
      case TokenType.Uint8: {
        const payload = await getPayload(1);
      ...
      case TokenType.Uint16: {
        const payload = await getPayload(2);
      ...
      case TokenType.Float64: {
        const payload = await getPayload(8);
      ...
      case TokenType.Float64Array: {
          ...
          const byteLength = lengthBytes.readUInt32LE(0);
          const data = await getPayload(byteLength, 5);
