...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // ⚡ Bolt: Speculative read-ahead optimization (16 bytes)
    // Minimizes asynchronous microtasks (awaits) by fetching type and common payload
    // lengths in a single I/O operation instead of separately.
    const SPECULATIVE_READ_SIZE = 16;
    let chunk = await this.readBytes(absoluteOffset, SPECULATIVE_READ_SIZE);
