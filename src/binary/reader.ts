...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Helper to check if we have enough data in buffer
    const ensureData = async (needed: number): Promise<Buffer> => {
       if (buffer.length >= needed) {
         return buffer.subarray(1, needed); // Skip type byte
       }
       return this.readBytes(absoluteOffset + 1n, needed - 1);
    };
    ...
    switch (type) {
      ...
      case TokenType.String: {
        const payload = await ensureData(5); // 1 type + 4 index
        if (payload.length < 4) {
      ...
      case TokenType.Number: {
        const lengthBytes = await ensureData(5);
      ...
      case TokenType.NumberRef: {
        const payload = await ensureData(5);
      ...
      case TokenType.Uint8: {
        const payload = await ensureData(2);
      ...
      case TokenType.Uint16: {
        const payload = await ensureData(3);
      ...
      case TokenType.Float64: {
        const payload = await ensureData(9);
