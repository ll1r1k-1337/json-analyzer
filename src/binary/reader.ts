...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Bolt Optimization: Speculative Read
    // Fetch enough bytes to cover most common fixed-width tokens (up to 16 bytes)
    // in a single read operation to minimize async overhead.
    // Note: When close to the end of the file, readBytes handles returning the remaining bytes safely.
