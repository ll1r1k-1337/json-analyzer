...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculative read: fetch up to 16 bytes to cover most fixed-width tokens without a second await
    const SPECULATIVE_READ_SIZE = 16;
    const specBuffer = await this.readBytes(absoluteOffset, SPECULATIVE_READ_SIZE);

    if (specBuffer.length < 1) {
      throw new Error("Unable to read token type");
    }

    const type = specBuffer.readUInt8(0) as TokenType;
    switch (type) {
      ...
      case TokenType.String: {
        let index;
        if (specBuffer.length >= 5) {
          index = specBuffer.readUInt32LE(1);
        } else {
      ...
      case TokenType.Number: {
        let byteLength;
        if (specBuffer.length >= 5) {
          byteLength = specBuffer.readUInt32LE(1);
        } else {
        ...
        let value;
        if (specBuffer.length >= 5 + byteLength) {
          value = specBuffer.subarray(5, 5 + byteLength).toString("utf8");
        } else {
      ...
      case TokenType.Uint8: {
        let val;
        if (specBuffer.length >= 2) {
          val = type === TokenType.Int8 ? specBuffer.readInt8(1) : specBuffer.readUInt8(1);
        } else {
          const payload = await this.readBytes(absoluteOffset + 1n, 1);
          if (payload.length < 1) throw new Error("Unable to read value");
          val = type === TokenType.Int8 ? payload.readInt8(0) : payload.readUInt8(0);
        }
        return { token: { type: TokenType.Number, value: String(val) }, byteLength: 2 };
      }
      case TokenType.Int16:
      case TokenType.Uint16: {
        let val;
        if (specBuffer.length >= 3) {
          val = type === TokenType.Int16 ? specBuffer.readInt16LE(1) : specBuffer.readUInt16LE(1);
        } else {
          const payload = await this.readBytes(absoluteOffset + 1n, 2);
          if (payload.length < 2) throw new Error("Unable to read value");
          val = type === TokenType.Int16 ? payload.readInt16LE(0) : payload.readUInt16LE(0);
        }
        return { token: { type: TokenType.Number, value: String(val) }, byteLength: 3 };
      }
      case TokenType.Int32:
      case TokenType.Uint32: {
        let val;
        if (specBuffer.length >= 5) {
          val = type === TokenType.Int32 ? specBuffer.readInt32LE(1) : specBuffer.readUInt32LE(1);
        } else {
          const payload = await this.readBytes(absoluteOffset + 1n, 4);
          if (payload.length < 4) throw new Error("Unable to read value");
          val = type === TokenType.Int32 ? payload.readInt32LE(0) : payload.readUInt32LE(0);
        }
        return { token: { type: TokenType.Number, value: String(val) }, byteLength: 5 };
      }
      case TokenType.Float64: {
        let val;
        if (specBuffer.length >= 9) {
          val = specBuffer.readDoubleLE(1);
        } else {
        ...
        return { token: { type: TokenType.Number, value: String(val) }, byteLength: 9 };
      }
      ...
      case TokenType.Float64Array: {
          ...
          let data;
          if (specBuffer.length >= 5 + byteLength) {
            data = Buffer.from(specBuffer.subarray(5, 5 + byteLength));
          } else {
