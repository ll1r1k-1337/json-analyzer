...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // ⚡ Bolt: Speculative read optimization
    // Read 16 bytes speculatively to cover common fixed-width tokens without extra await overhead
    ...
    switch (type) {
      ...
      case TokenType.String: {
        let payload = chunk.length >= 5 ? chunk.subarray(1, 5) : await this.readBytes(absoluteOffset + 1n, 4);
        if (payload.length < 4) {
      ...
      case TokenType.Number: {
        let lengthBytes = chunk.length >= 5 ? chunk.subarray(1, 5) : await this.readBytes(absoluteOffset + 1n, 4);
      ...
      case TokenType.Uint8: {
        let payload = chunk.length >= 2 ? chunk.subarray(1, 2) : await this.readBytes(absoluteOffset + 1n, 1);
      ...
      case TokenType.Uint16: {
        let payload = chunk.length >= 3 ? chunk.subarray(1, 3) : await this.readBytes(absoluteOffset + 1n, 2);
      ...
      case TokenType.Float64: {
        let payload = chunk.length >= 9 ? chunk.subarray(1, 9) : await this.readBytes(absoluteOffset + 1n, 8);
      ...
      case TokenType.Float64Array: {
          ...
          let data;
          if (chunk.length >= 5 + byteLength) {
             data = chunk.subarray(5, 5 + byteLength);
          } else {
