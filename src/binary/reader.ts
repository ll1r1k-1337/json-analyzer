...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculative read: fetch enough bytes for most common tokens to avoid multiple I/O calls
    const SPECULATIVE_READ_SIZE = 16;
    const buffer = await this.readBytes(absoluteOffset, SPECULATIVE_READ_SIZE);
    if (buffer.length < 1) {
    ...
    switch (type) {
      ...
      case TokenType.Number: {
        ...
        if (buffer.length >= 5 + byteLength) {
           numberBytes = buffer.subarray(5, 5 + byteLength);
        } else {
      ...
      case TokenType.Float64Array: {
          ...
          let data: Buffer;
          if (buffer.length >= 5 + byteLength) {
             data = buffer.subarray(5, 5 + byteLength);
          } else {
