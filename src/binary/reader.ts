...
export class BinaryTokenReader {
  ...
  async readTokenAt(offset: bigint): Promise<BinaryTokenResult> {
    ...
    // Speculatively read 16 bytes to cover most token types in a single I/O operation
    // This significantly reduces the overhead of asynchronous microtasks.
    ...
    switch (type) {
      ...
      case TokenType.Number: {
        ...
        if (byteLength <= 11 && specBuffer.length >= 5 + byteLength) { // 16 - 5 = 11 bytes max inline
          numberBytes = specBuffer.subarray(5, 5 + byteLength);
        } else {
      ...
      case TokenType.Uint8: {
        ...
        if (specBuffer.length >= 2) {
          payload = specBuffer.subarray(1, 2);
        } else {
          payload = await this.readBytes(absoluteOffset + 1n, 1);
      ...
      case TokenType.Uint16: {
        ...
        if (specBuffer.length >= 3) {
          payload = specBuffer.subarray(1, 3);
        } else {
          payload = await this.readBytes(absoluteOffset + 1n, 2);
      ...
      case TokenType.Uint32: {
        let payload: Buffer;
        if (specBuffer.length >= 5) {
          payload = specBuffer.subarray(1, 5);
        } else {
          payload = await this.readBytes(absoluteOffset + 1n, 4);
      ...
      case TokenType.Float64: {
        ...
        if (specBuffer.length >= 9) {
          payload = specBuffer.subarray(1, 9);
        } else {
          payload = await this.readBytes(absoluteOffset + 1n, 8);
      ...
      case TokenType.Float64Array: {
          ...
          let data: Buffer;
          if (byteLength <= 11 && specBuffer.length >= 5 + byteLength) {
            data = specBuffer.subarray(5, 5 + byteLength);
          } else {
