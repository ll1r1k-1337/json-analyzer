...
export class BinaryTokenWriter implements BinaryWriter {
  ...
  private registerString(value: string): number {
    ...
    if (
      this.options?.maxUniqueStrings &&
      this.stats.strings.uniqueCount + 1 > this.options.maxUniqueStrings
    ) {
      throw new Error(
        `String table limit exceeded: unique count (${this.options.maxUniqueStrings})`
      );
    }

    if (
      this.options?.maxStringTableBytes &&
      this.stats.strings.uniqueBytes + byteLength >
        this.options.maxStringTableBytes
    ) {
      throw new Error(
        `String table limit exceeded: byte size (${this.options.maxStringTableBytes})`
      );
    }
