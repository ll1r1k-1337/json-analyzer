...
export class BinaryTokenWriter implements BinaryWriter {
  ...
  private registerString(value: string): number {
    ...
    if (
      ...
      this.strings.length >= this.options.maxUniqueStrings
    ) {
      throw new Error(
        `String table limit reached: ${this.options.maxUniqueStrings} unique strings`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `String table size limit reached: ${this.options.maxStringTableBytes} bytes`
      );
    }
