...
export class BinaryTokenWriter implements BinaryWriter {
  ...
  private registerString(value: string): number {
    ...
    if (this.options?.maxUniqueStrings && this.stats.strings.uniqueCount >= this.options.maxUniqueStrings) {
      throw new Error(`String table unique count limit reached: ${this.options.maxUniqueStrings}`);
    }

    if (this.options?.maxStringTableBytes && this.stats.strings.uniqueBytes + byteLength > this.options.maxStringTableBytes) {
      throw new Error(`String table size limit reached: ${this.options.maxStringTableBytes} bytes`);
    }
